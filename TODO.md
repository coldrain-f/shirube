...
- [ ] 대기열 데이터 업데이트 버튼(표제어, 요미가나, 뜻, 품사 등)
- [ ] 대기열 데이터(단어) 삭제 버튼
- [ ] 대기열 데이터 사전에 등록 오류 해결
- [ ] 대기열 데이터 일괄 사전에 등록 기능
- [ ] 사전 선택 기능(대기열에서 사전에 등록할 때 사전을 선택할 수 있도록)
- [ ] 사전이 여러 개로 설계가 변경됨에 따른 대기열 단어 목록 조회, 내보내기, 불러오기, 대기열 초기화 기능 수정 및 DB 구조 확인 및 변경
- [ ] 사전이 여러 개로 설계가 변경됨에 따른 별도 사전 관리 화면 개발 필요(Yomitan, Kindle로 만들어질 사전 등록 및 수정, 삭제 기능.)
